...
from rlbot425.convert.pyconv import PyConv
...
class Bot:
    ...
    def convert_to_toml(self) -> dict[str, dict[str, Any]]:
        ...
        bot_name: str = toml_dict["settings"]["name"].replace(" ", "-")
        dev_name: str = toml_dict["details"].get("developer", "developer-name")  # type: ignore
        dev_name = dev_name.replace(" ", "-").lower()
        toml_dict["settings"]["agent_id"] = f"{dev_name}/{bot_name}"
    ...
    def write_to_toml(self, bot_path: Path):
        ...
        old_loadout: str = config["settings"]["loadout_file"]
        Loadout(self.parent_path / old_loadout.replace(".toml", ".cfg")).write_to_toml(
            bot_path.parent / old_loadout
        )

    def convert_python(self, v5_base_dir: Path):
        pyconv = PyConv(self.parent_path / self.cfg_dict["Locations"]["python_file"])
        pyconv.convert(v5_base_dir)
